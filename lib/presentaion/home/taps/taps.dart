...
class NewsTaps extends StatefulWidget {
  NewsTaps({super.key, required this.viewModel, this.article});
        ...
        Consumer<HomeViewModel>(
          builder: (context, viewModel, child) {
            if (viewModel.sources.isEmpty) {
              return const Center(
                child: CircularProgressIndicator(), // Show loading if no sources
              );
            }
              ...
              child: Column(
                children: [
                  TabBar(
                    ...
                    tabs: viewModel.sources.map((source) {
                      return Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: Text(
                            source.name.toString()), // Display source name
                      );
                    }).toList(),
                    onTap: (index) {
                      setState(() {
                        _selectedTabIndex =
                            index; // Ensure _selectedTabIndex is defined
                      });
                    },
                  ),
                  Expanded(
                    child: TabBarView(
                      children: viewModel.sources.map((source) {
                        return Center(
                          child: Text(
                              "Content for ${source.name}"), // Sample content
                        );
                      }).toList(),
                    ),
                  ),
...
//                         sourceId: sources[_selectedTabIndex].id),
